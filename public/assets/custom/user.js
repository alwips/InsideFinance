var User = (function() {
    ...
    var base_url = 'http://'+location.host+'/user';
    $.ajaxSetup({
        dataType: 'JSON',
        headers: {
        ...
        $('#dt-ajax-object').DataTable({
            ...
            "columns": [
                ...
                { "data": "id", "render": function(id, type, full, meta) {
                    return `<a href="`+base_url+`/req/`+id+`/edit" class="icofont icofont-ui-edit"></a> | \
                    <a href="#delete"><i class="icofont icofont-ui-delete"></i></a> | \
                    <a href="#detail"><i class="icofont icofont-eye-alt"></i></a> | \
                    <a href="#reset"><i class="icofont icofont-ui-password"></i></a>
                    `;
                }}
            ],
          "order": [[3, 'asc']]
    ...
    aksi.reset = () => {
        $('#dt-ajax-object tbody').on('click','a[href="#reset"]', function(e){
            e.preventDefault(); // prevent page refresh
            var thisDetail = $(this);
            ...
            swal({
                ...
                if (inputValue === "") {
                    swal.showInputError("Silahkan masukan password baru!");
                    return false
                }
                $.ajax({
                    url: base_url+'/req/'+id,
                    type: 'POST',
                    data: {_method:'PATCH',password:inputValue},
                    success: function (data) {
                        if (data.status == 'success') {
                            setTimeout(function () {
                            ...
                            setTimeout(function () {
                                swal("Gagal!", "Password tidak berhasil di reset !", "error");
                            }, 2000);
                        }
                    },
                    error: function (data) {
                        setTimeout(function () {
                            swal("Gagal!", "Password tidak berhasil di reset !", "error");
                            notify('Password gagal di proses, silahkan hubungi Technical Support ', 'danger');
                        }, 2000);
                    }
                });
            });
